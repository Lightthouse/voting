...
```solidity
mapping(string &#x3D;&gt; struct Elections.ElectionCampaign) electionCampaign
```

### voteCommission

```solidity
uint256 voteCommission
```
...
```solidity
modifier campaignRequire(string _name)
```

### campaignUniq

```solidity
modifier campaignUniq(string _name)
```

### campaignExpired

```solidity
modifier campaignExpired(string _name)
```

### votingAllow

```solidity
modifier votingAllow(string _campaignName, uint256 _candidateNumber)
```
...
```solidity
function getCandidatesList(string _name) external view returns (address, address, address)
```
...
### getCandidateVotesCount

```solidity
function getCandidateVotesCount(string _campaignName, uint256 _candidateNumber) external view returns (uint256)
```
...
```solidity
function getVoteCommission() external view returns (uint256)
```
...
```solidity
function getElectionInfo(string _name) external view returns (uint256, uint256)
```
...
```solidity
function getElectionBalance(string _name) external view returns (uint256)
```

Election campaign balance.

### createElectionCampaign

```solidity
function createElectionCampaign(string _name, address _candidate_1, address _candidate_2, address _candidate_3) external
```

Create new election campaign.

_only 3 candidates allowed._

### vote

```solidity
function vote(string _campaignName, uint256 _candidateNumber) external payable
```
...
```solidity
function finishElection(string _name) external
```

Finnish election campaign and send money to the winner.

_There is no draw. Only one candidate can get money._
